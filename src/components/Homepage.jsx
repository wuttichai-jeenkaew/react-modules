...
export function FAQ() {
 ...
 const Demo = () => {
  return (
    <Accordion.Root collapsible defaultValue={["a"]} w="70%">
      {items.map((item, index) => (
        <Accordion.Item key={index} value={item.value}>
          ...
          <Accordion.ItemContent>
            <Accordion.ItemBody pb={4} className="break-words" fontSize="lg">{item.text}</Accordion.ItemBody>
          </Accordion.ItemContent>
        </Accordion.Item>
      ))}
    </Accordion.Root>
...
const items = [
  { value: "a", title: "Q: What makes XYZ Company's products so unique?", 
